...
PigeonInitializeResponse::PigeonInitializeResponse(
 ...
 : name_(name),
    options_(std::make_unique<PigeonFirebaseOptions>(options)),
...
PigeonInitializeResponse::PigeonInitializeResponse(const PigeonInitializeResponse& other)
 : name_(other.name_),
    options_(std::make_unique<PigeonFirebaseOptions>(*other.options_)),
...
PigeonInitializeResponse& PigeonInitializeResponse::operator=(const PigeonInitializeResponse& other) {
  name_ = other.name_;
  options_ = std::make_unique<PigeonFirebaseOptions>(*other.options_);
...
void PigeonInitializeResponse::set_options(const PigeonFirebaseOptions& value_arg) {
  options_ = std::make_unique<PigeonFirebaseOptions>(value_arg);
}
...
PigeonInitializeResponse PigeonInitializeResponse::FromEncodableList(const EncodableList& list) {
  PigeonInitializeResponse decoded(
    std::get<std::string>(list[0]),
    std::any_cast<const PigeonFirebaseOptions&>(std::get<CustomEncodableValue>(list[1])),
